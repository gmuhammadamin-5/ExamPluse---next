...
const getColor = (id) => ['#007AFF','#34C759','#AF52DE','#FF9500','#00C7BE','#FF2D55','#5AC8FA','#FF6B6B'][Math.abs(id||0)%8]
...
export default function GlobalChat() {
  ...
  const realName = user?.full_name || `${profile.firstName} ${profile.lastName}`.trim() || 'You'
  const realInitials = user?.full_name
    ? user.full_name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2)
    : (profile.firstName?.[0] || '') + (profile.lastName?.[0] || '') || 'Me'
  const ME = { id: user?.id || 99, name: realName, avatar: realInitials, online: true, lastSeen: 'Online' }

  const [view, setView]           = useState('global')
  const [globalMsgs, setGMsgs]   = useState([])
  const [dmMsgs, setDMsgs]       = useState({})
  ...
  const msgSenders = useRef({})
  const lastCount  = useRef(0)

  // ── FETCH USERS ──
  useEffect(() => {
    authFetch('/api/chat/users').then(r => r.ok ? r.json() : []).then(users => {
      setRealUsers(users)
      users.forEach(u => { msgSenders.current[u.id] = u })
    }).catch(() => {})
  }, [])

  // ── POLL GLOBAL MESSAGES every 3s ──
  useEffect(() => {
    if (view !== 'global') return
    const fetchGlobal = () => {
      authFetch('/api/chat/global').then(r => r.ok ? r.json() : null).then(data => {
        if (!data) return
        data.forEach(m => {
          if (m.sender) msgSenders.current[m.sender.id] = { id: m.sender.id, name: m.sender.name, avatar: m.sender.avatar, online: false }
        })
        setGMsgs(prev => {
          const normalized = data.map(m => ({
            id: m.id,
            uid: m.isMe ? ME.id : m.sender.id,
            text: m.text,
            time: m.time,
            r: prev.find(p => p.id === m.id)?.r || {},
            pinned: prev.find(p => p.id === m.id)?.pinned || false,
            isMe: m.isMe,
            deleted: m.deleted,
          }))
          // Keep local-only messages (tmp- ids) that haven't synced yet
          const tmpMsgs = prev.filter(m => String(m.id).startsWith('tmp-'))
          return [...normalized, ...tmpMsgs]
        })
      }).catch(() => {})
    }
    fetchGlobal()
  ...
  }, [globalMsgs.length, activeDM, view])
  ...
  // ── FAKE TYPING indicator (UX polish) ──
  useEffect(() => {
    if (view !== 'global') return
    const t1 = setTimeout(() => setTyping(true), 8000)
    const t2 = setTimeout(() => setTyping(false), 10500)
  ...
  const getUser = (id) => {
    ...
    return msgSenders.current[id] || realUsers.find(u => u.id === id) || { id, name: 'Foydalanuvchi', avatar: 'F', online: false }
  }
  ...
  const send = useCallback(async () => {
    ...
    if (view === 'global') {
      ...
      setGMsgs(p => [...p, { id: tmpId, uid: ME.id, text, time: 'Hozir', r: {}, pinned: false, isMe: true, ...(replyTo ? { replyTo } : {}) }])
      authFetch('/api/chat/global', { method: 'POST', body: JSON.stringify({ text }) }).catch(() => {})
    } else if (activeDM) {
      const tmpId = `tmp-${Date.now()}`
      setDMsgs(p => ({ ...p, [activeDM]: [...(p[activeDM] || []), { id: tmpId, from: 'me', text, time: 'Hozir', read: false }] }))
      authFetch(`/api/chat/dm/${activeDM}`, { method: 'POST', body: JSON.stringify({ text }) }).catch(() => {})
    }
  }, [input, editingId, editText, replyTo, view, activeDM, ME.id])
  ...
  const deleteMsg = (id) => {
    ...
    setGMsgs(p => p.map(m => m.id === id ? { ...m, deleted: true, text: '' } : m))
    authFetch(`/api/chat/global/${id}`, { method: 'DELETE' }).catch(() => {})
  }
  ...
  const stopRecord = () => {
    ...
    const dur = recSec; setRecSec(0)
    const msg = { id: `tmp-${Date.now()}`, uid: ME.id, isMe: true, time: 'Hozir', r: {}, pinned: false, voice: true, duration: dur }
  ...
  const handleImage = (file) => {
    ...
    reader.onload = (e) => {
      const msg = { id: `tmp-${Date.now()}`, uid: ME.id, isMe: true, time: 'Hozir', r: {}, pinned: false, img: e.target.result }
      if (view === 'global') setGMsgs(p => [...p, msg])
      else if (activeDM) setDMsgs(p => ({ ...p, [activeDM]: [...(p[activeDM] || []), { ...msg, from: 'me', read: false }] }))
    }
  ...
  const openDM = (id) => {
    setActiveDM(id); setView('dm')
    authFetch(`/api/chat/dm/${id}`).then(r => r.ok ? r.json() : []).then(msgs => {
      setDMsgs(p => ({ ...p, [id]: msgs }))
    }).catch(() => {})
  }
  ...
  const filtered   = realUsers.filter(u => u.id !== ME.id && (u.name.toLowerCase().includes(searchQ.toLowerCase())))
  ...
  const Bubble = ({ msg, isDM = false }) => {
    ...
    const sender = isMe ? ME : getUser(msg.uid)
        ...
        <div style={{ width: 30, flexShrink: 0, display: 'flex', alignItems: 'flex-end' }}>
          {isMe
            ? (profile.avatar
              ...
              : <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(145deg,#007AFF,#5856D6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#fff' }}>{realInitials}</div>)
  ...
  const onlineUsers = realUsers.filter(u => u.online || u.id === ME.id)
        ...
        <div style={{ width: 68, background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(20px)', borderRight: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '14px 0', gap: 4 }}>
          ...
          <div style={{ width: 40, height: 40, borderRadius: '50%', overflow: 'hidden', cursor: 'pointer' }} onClick={() => window.location.href = '/profile'}>
            {profile.avatar
              ...
              : <div style={{ width: '100%', height: '100%', background: 'linear-gradient(145deg,#007AFF,#5856D6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#fff' }}>{realInitials}</div>
            }
          </div>
        </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px 8px' }}>
            ...
            {view === 'global' && <>
              <div style={{ display: 'flex', gap: 12, padding: '4px 6px 14px', overflowX: 'auto' }}>
                {(onlineUsers.length ? onlineUsers : [ME]).map(u => (
                  <div key={u.id} onClick={() => u.id !== ME.id && openDM(u.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: u.id !== ME.id ? 'pointer' : 'default', flexShrink: 0 }}>
              ...
              <div className="gc-row" onClick={() => setView('global')} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px', background: 'rgba(0,122,255,0.07)' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: '#34C759', fontWeight: 500 }}>{realUsers.length || '...'} foydalanuvchi · Faol</div>
                </div>
            ...
            {view === 'people' && <>
              ...
              <div style={{ fontSize: 11, fontWeight: 600, color: '#8E8E93', padding: '12px 6px 6px' }}>PLATFORM FOYDALANUVCHILARI</div>
              {filtered.filter(u => !friends.includes(u.id) && !requests.find(r => r.id === u.id)).map(u => (
                <div key={u.id} className="gc-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px' }}>
                  <Avatar user={u} size={44} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 500, color: '#1C1C1E' }}>{u.name}</div>
                    <div style={{ fontSize: 12, color: '#8E8E93' }}>{u.online ? 'Online' : 'Faol'}</div>
                  </div>
              ...
              {filtered.length === 0 && realUsers.length === 0 && (
                <div style={{ textAlign: 'center', padding: '40px 16px', color: '#8E8E93', fontSize: 13 }}>Yuklanmoqda...</div>
              )}
            </>}
                    ...
                    <div key={u.id} style={{ background: 'rgba(255,255,255,0.80)', borderRadius: 14, padding: '14px', marginBottom: 6 }}>
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: '#8E8E93' }}>ExamPulse foydalanuvchisi</div>
                        </div>
                      </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          {(view === 'global' || activeDM) && <>
              ...
              {view === 'global' ? (
                <>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#34C759', fontWeight: 500 }}>{realUsers.length || '...'} foydalanuvchi</div>
                  </div>
                ...
                <>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#8E8E93', fontWeight: 500 }}>Shaxsiy xabar</div>
                  </div>
            ...
            <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 2, position: 'relative' }}>
              ...
              {globalMsgs.length === 0 && view === 'global' && (
              ...
              {view === 'dm' && activeDM && (dmMsgs[activeDM] || []).map(msg => (
                <div key={msg.id} className="gc-msg"><Bubble msg={msg} isDM /></div>
              ))}
            ...
            {showScrollBtn && (
              <button onClick={scrollToBottom} style={{
                ...
                boxShadow: '0 4px 16px rgba(0,0,0,0.12)', zIndex: 20, color: '#007AFF',
              }}>
                <ChevronDown size={18} />
              </button>
            )}
